    ...
    return [
      {
        header: (head: HeaderContext<TTableColumns, TTableColumns>) => (
          <td
            className={`w-[23%] ${key === "eventCode" && !isRtl ? "ml-4" : "px-2"} `}
          >
            <div
              className={
        ...
        cell: (row: {
          ...
          // Update the className to use the condition for value
          const className = `py-3.5 font-readexProRegular leading-4 text-sm justify-start whitespace-nowrap break-all overflow-hidden text-overflow-ellipsis w-[23%] text-black flex text-wrap ${
            key === "eventCode" && !isRtl ? "ml-4" : "px-2"
          }`;
